...
This is a standalone, language-agnostic code cleanup tool that uses Codex and optionally Codex to find and remove dead code, simplify complexity, and clean up technical debt.
...
.Codex/commands/              # Codex slash commands (usable interactively)
...
4. **Self-review** — Codex reviews its own diff
...
The `.Codex/commands/` directory contains interactive commands. When working inside this project (or any project that symlinks these commands), they're available as `/gc`, `/review`, `/self-review`, etc.
